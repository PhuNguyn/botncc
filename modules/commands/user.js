...
module.exports.run = async ({ event, api, args, Users, client }) => {
	...
	switch (args[0]) {
      ...
      if (!mention){
        ...
        let dataUser = (await Users.getData(mention.toString()));
				if (!dataUser) return api.sendMessage(`[${mention}] người dùng không tồn tại trong database!`, event.threadID);
				if (dataUser.banned) return api.sendMessage(`[${mention}] Người dùng đã bị ban từ trước`, event.threadID);
				return api.sendMessage(`[${mention}] Bạn muốn ban người dùng này ?\n\nHãy reaction vào tin nhắn này để ban!`, event.threadID, (error, info) => {
					client.handleReaction.push({
						name: "user",
						messageID: info.messageID,
						author: event.senderID,
        ...
        case "banlist": {
            var data = await Users.getAll(['name', 'userID'], {banned: 1});
            var userBlockMsg = "";
            data.forEach(user => userBlockMsg += `\n${user.name} - ${user.userID}`);
            api.sendMessage((userBlockMsg) ? `🛠 | Đây là danh sách các user bị ban:${userBlockMsg}` : 'Chưa có user nào bị ban!', event.threadID, event.messageID);
			break;
		}
		case "search": {
