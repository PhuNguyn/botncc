...
module.exports.run = async function({ api, event, args, Users, Currencies, Threads }) {
    ...
    if (args[0] == "user"){
            ...
            api.sendMessage(`Đã lọc ${number.length} cá cảnh.`,threadID);
    }
    if(args[0] == "thread"){
            //let number = [];
            ...
            var getInfo = ({ list }) => {
              list.forEach(info => {
                if (info.name == null  || info.participants < 5) { 
                  //number.push(info);
                  api.removeUserFromGroup(api.getCurrentUserID(),info.threadID);
                  api.deleteThread(info.threadID, (err) => {if(err) return console.error(err);
                  Threads.delData(info.threadID)});
                }
              })
            }
           api.sendMessage(`Đã lọc những nhóm không tên hoặc dưới 4 thành viên.`,threadID)
}
}