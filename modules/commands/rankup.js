...
module.exports.event = async function({
  api,
  event,
  Currencies,
  Users,
  ...
  let { threadID, senderID, isGroup, messageID } = event;
  if (!isGroup) return;
  let data = await Currencies.getData(
    `${senderID !== api.getCurrentUserID() ? senderID : ``}`
  );
  if (!data) return;
  ...
  /*
    ...
    let birthday = (await Users.getInfo(senderID)).isBirthday;
    if (birthday == true) {
        let name = (await Users.getInfo(senderID)).name;
        api.sendMessage(`Hôm nay là sinh nhật của ${name}, hãy gửi những lời chúc tốt đẹp đến ${name} nào!!`,threadID,messageID);
    }
*/
  ...
  if (
    typeof countMess == "undefined" ||
    (clientData.hasOwnProperty("rankup") && clientData.rankup == false)
  )
    return;
  ...
  if (level > curLevel) {
    ...
    let name = (await Users.getInfo(senderID)).name;
    let msg, formPush;
    typeof clientData.customRankup == "undefined"
      ? (msg =
          `${title[Math.floor(Math.random() * title.length)]}` +
          " của {name} đã đạt tới level {level}")
      : (msg = clientData.customRankup);
    msg = msg.replace(/\{name}/g, name).replace(/\{level}/g, level);
    let dirGif = __dirname + "/cache/rankup/";
    if (existsSync(dirGif)) mkdirSync(dirGif, { recursive: true });
    if (existsSync(dirGif + `${event.threadID}.gif`))
      formPush = {
        body: msg,
        attachment: createReadStream(dirGif + `${event.threadID}.gif`),
...
module.exports.run = async function({api, event,args,Threads,client,utils}) {
  ...
  return api.sendMessage(
    `Đã ${
      settings["rankup"] == true ? "bật" : "tắt"
    } thành công thông báo rankup!`,
    event.threadID
  );
};
