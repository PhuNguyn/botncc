module.exports = function({ api, __GLOBAL, client, models, Users, Threads, Currencies, utils }) {
	...
	return async function({ event }) {
		...
		var permssion;
		if (command.config.hasPermssion == 2 && !__GLOBAL.settings.ADMINBOT.includes(senderID)) return api.sendMessage(`❌ Bạn không đủ quyền hạn người điều hành bot để sử dụng lệnh ${command.config.name}`, threadID, messageID);
		else permssion = 2;
		...
		if (command.config.hasPermssion == 1 && !__GLOBAL.settings.ADMINBOT.includes(senderID) && !find) return api.sendMessage(`❌ Bạn không đủ quyền hạn quản trị viên để sử dụng lệnh ${command.config.name}`, threadID, messageID);
		else permssion = 1;
